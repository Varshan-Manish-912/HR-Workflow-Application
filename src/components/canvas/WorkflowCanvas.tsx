...
function FlowContent({
    ...
    const onConnect = useCallback(
        (connection: Connection) => {
            ...
            if (sourceNode?.type === "approval") {
                ...
                if (existingOutgoing.length === 0) label = "approved";
                else if (existingOutgoing.length === 1) label = "rejected";
                else {
    ...
    const getDefaultData = (type: string) => {
        switch (type) {
            ...
            case "approval":
                return { label: "", role: "", threshold: undefined };
            case "automated":
                return { label: "" };
            case "end":
                return { label: "" };
    ...
    const highlightedEdges = edges.map((edge) => {
        ...
        return {
            ...
            style: isActive
                ...
                : { stroke: "#e5e7eb", strokeWidth: 1.5, opacity: 0.8 },
            animated: isActive,
        };
    });
    ...
    return (
        <div ref={reactFlowWrapper} className="relative w-full h-full">
            <div className="absolute z-10 top-4 right-4">
                <div className="flex items-center gap-1 p-1 border rounded-xl bg-[#111827]/90 backdrop-blur-md border-[#1f2937] shadow-lg">
                    <button
                        onClick={undo}
                        disabled={!canUndo}
                        className="flex items-center justify-center w-8 h-8 text-white transition-all duration-150 rounded-lg hover:bg-[#1f2937] active:bg-[#374151] disabled:opacity-30 disabled:cursor-not-allowed active:scale-90"
                    >
                        <Undo2 size={16} />
                    </button>
...
function CanvasWithPanel() {
    ...
    const renderForm = () => {
        ...
        switch (selectedNode.type) {
            ...
            default:
                return (
                    <p className="text-sm text-gray-400">No config available</p>
                );
        }
    };
        ...
        <PanelGroup direction="horizontal" className="h-full bg-canvas">
            <Panel defaultSize={75} minSize={50}>
                <PanelGroup direction="vertical">
                    ...
                    <PanelResizeHandle className="h-1 bg-gray-700 cursor-row-resize hover:bg-gray-500" />
            ...
            <PanelResizeHandle className="w-1 bg-gray-700 cursor-col-resize hover:bg-gray-500" />
            ...
            >
                <PanelGroup direction="vertical">
                    <Panel defaultSize={60} minSize={30} className="overflow-hidden">
                        <div className="relative flex flex-col h-full p-4 overflow-hidden bg-panel">
                            <button
                            ...
                            {selectedNode ? (
                                <div className="flex-1 px-1 overflow-y-auto overflow-x-hidden">
                                    {renderForm()}
                                </div>
                            ) : (
